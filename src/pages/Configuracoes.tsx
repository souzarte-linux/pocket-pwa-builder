...
export const Configuracoes = () => {
  ...
  useEffect(() => {
    if (profile) {
      setForm({
        ...
        daily_goal: profile.daily_goal?.toString() ?? '',
        weekly_goal: profile.weekly_goal?.toString() ?? '',
        ...
        plate: profile.plate ?? '',
        tank_size_l: profile.tank_size_l?.toString() ?? '',
        avg_consumption_kml: profile.avg_consumption_kml?.toString() ?? '',
        oil_change_km: profile.oil_change_km?.toString() ?? '',
        tire_size_front: profile.tire_size_front ?? '',
        tire_size_rear: profile.tire_size_rear ?? '',
        has_bag: profile.has_bag ? 'true' : 'false',
      });
    }
  }, [profile]);
  ...
  return (
    <AppShell title={'CONFIGURAÇÕES'} back>
      {isLoading ? (
        <div className="py-12 text-center text-[#ab8a7d] flex items-center justify-center gap-2">
        ...
        <div className="py-12 text-center text-red-400">
          <p className="text-sm font-semibold">Erro ao carregar configurações do sistema.</p>
        </div>
      ) : (
        <form onSubmit={onSubmit}>
          <div className="rounded-3xl border border-primary-container/30 bg-surface-container p-5 shadow-[0_20px_80px_rgba(255,95,0,0.12)] [&_label]:text-primary-container [&_label]:uppercase [&_label]:font-semibold [&_label]:text-[10px] [&_label]:tracking-[0.18em] [&_input]:!border-primary-container/30 [&_input]:!focus:border-primary-container [&_input]:!bg-surface-container [&_select]:!border-primary-container/30 [&_select]:!focus:border-primary-container [&_select]:!bg-surface-container">
            <FormShell footer={<SubmitButton loading={isUpdating}>SALVAR</SubmitButton>}>
