...
const Apps = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setPlatforms((ps ?? []).map((p) => ({ ...p, total: totals.get(p.id) ?? 0 })));
    };
    load();
  }, []);
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                ...
                {p.active && (
                  <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-secondary text-secondary-foreground">
                    ATIVO
                  </span>
                )}
              </div>
            ...
            <button
              ...
              className="size-11 grid place-items-center rounded-xl bg-surface-high text-foreground hover:bg-surface-highest"
