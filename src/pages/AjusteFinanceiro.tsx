...
const AjusteFinanceiro = () => {
  ...
  const { createAdjustment } = useFinancialAdjustmentMutations();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(false);
      return toast.error(err?.message || 'Erro ao registrar ajuste financeiro');
    }
  };
        ...
        <FormShell footer={<SubmitButton loading={loading}>REGISTRAR AJUSTE ›</SubmitButton>}>
          ...
          <Field label="Descrição (Opcional)">
            <Input
              ...
              placeholder="Ex: Carga extraviada, Bônus de domingo..."
            />
          </Field>
          ...
          <div className="rounded-xl border-l-4 border-info bg-surface p-4 flex gap-3 text-sm text-muted-foreground mt-4">
            ...
            <p>
              Este ajuste ficará pendente e será automaticamente incluído e abatido/somado na próxima{' '}
              <strong>Fatura</strong> que você fechar para esta plataforma.
            </p>
          </div>
        </FormShell>
      </form>
